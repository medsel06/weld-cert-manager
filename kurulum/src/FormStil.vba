...
' =====================================================================
'  FORM STİL MOTORU  (DPI ölçekleme + ekrana sığdırma + kurumsal tema)
'  ---------------------------------------------------------------
'  AMAÇ: UserForm'lar yüksek-DPI (4K / %150) laptoplarda küçük çizilir
'        (VBA formları DPI'a duyarlı değildir). Bu motor her formu:
'          1) İÇERİĞE GÖRE boyutlandırır  -> iç taşma / yarım kontrol /
'             gereksiz boşluk olmaz.
'          2) Ekranın DPI'ına göre orantılı büyütür (monitör görünümü
'             korunur, laptopta okunur olur).
'          3) EKRANI AŞMAYACAK şekilde sınırlar; sığmazsa kaydırma
'             çubuğu açar -> hiçbir kontrol ekran dışında kalmaz.
'          4) Kurumsal renk teması + Segoe UI uygular, formu ortalar.
'
'  KURULUM: FormStil.bas'i içe aktarın; 4 formun UserForm_Initialize
'           olayının İLK satırına şunu ekleyin:  FormStil.Uygula Me
'           (FORM_STIL_KURULUM.md dosyasına bakın.)
'
'  AYAR: OLCEK_ZORLA = 0 (otomatik DPI). Elle istiyorsanız 1.0-2.0 verin.
' =====================================================================

Private Const OLCEK_ZORLA As Double = 0       ' 0 = otomatik DPI
Private Const OLCEK_TABAN As Double = 1#       ' istek faktörü en az 1 (küçültmeyi yalnız ekran sınırı yapar)
Private Const OLCEK_TAVAN As Double = 1.8       ' aşırı büyütmeyi engelle
Private Const OLCEK_DIP As Double = 0.6         ' ekrana sığdırırken bundan fazla küçültme (altında scrollbar)
Private Const MIN_PUNTO As Double = 11#          ' hedef minimum punto (DPI=100 iken)
Private Const KENAR As Double = 14#              ' form iç kenar boşluğu (punto)
Private Const EKRAN_PAY_X As Double = 0.94       ' ekran genişliğinin kullanılabilir oranı
Private Const EKRAN_PAY_Y As Double = 0.9        ' ekran yüksekliğinin kullanılabilir oranı
...
' =====================================================================
'  TEK GİRİŞ NOKTASI - her formun Initialize olayından çağrılır.
' =====================================================================
Public Sub Uygula(frm As Object)
    ...
    ' 1) Ölçeksiz içerik kutusu (tüm kontrolleri saran sınır)
    Dim baseR As Double
    Dim baseB As Double
    IcerikKutusu frm, baseR, baseB
    If baseR < 1 Or baseB < 1 Then Exit Sub

    ' 2) İstenen büyütme (DPI + minimum punto)
    Dim istenen As Double
    istenen = IstenenOlcek(frm, dpi)

    ' 3) Ekran sınırı (punto): formu buraya sığdır
    Dim ekrW As Double
    Dim ekrH As Double
    ...
    Dim final As Double
    final = istenen
    If final > ekranMax Then final = ekranMax     ' ekrana sığdırmak için gerekirse küçült
...
' ---------------------------------------------------------------------
'  İstenen ölçek = max(DPI/96, MIN_PUNTO/en-küçük-font, OLCEK_TABAN)
' ---------------------------------------------------------------------
...
' Ekran boyutunu punto cinsine çevir (mantıksal piksel 96 tabanlıdır)
...
' ---------------------------------------------------------------------
'  Tüm kontrolleri tek faktörle ölçekle (oran korunur, iç taşma olmaz)
' ---------------------------------------------------------------------
...
' ---------------------------------------------------------------------
'  Formu içeriğe göre boyutla; ekranı aşarsa kırp + kaydırma çubuğu aç.
' ---------------------------------------------------------------------
Private Sub FormaSigdir(frm As Object, ByVal icerikW As Double, ByVal icerikH As Double, _
    ...
    If gerW > icW + 1 Or gerH > icH + 1 Then
        ...
        frm.KeepScrollBarsVisible = 0      ' yalnız gerekince göster
...
' ---------------------------------------------------------------------
'  Kurumsal tema: yazı tipi, renkler, rol bazlı buton/etiket biçimi.
' ---------------------------------------------------------------------
